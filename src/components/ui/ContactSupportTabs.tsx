  ...
  return (
    <section className="bg-transparent py-10 md:py-10">
      <div className="max-w-7xl mx-auto px-4 md:px-15">
        {/* Desktop Tabs */}
        <div ref={tabsRef} className="hidden md:flex justify-center mb-14">
          <div className="flex rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.1)] w-full max-w-225 gap-4">
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`w-full rounded-full text-sm md:text-lg lg:text-xl py-4 cursor-pointer transition-all duration-200
                  ${
                    activeTab === tab
        ...
        <div className="block md:hidden mb-10">
          ...
          >
            <SelectTrigger className="w-full h-12 border-2 border-[#FFFFFF]/50 rounded-full text-white px-5 py-3 bg-[#202020]">
            ...
            >
              {activeTab === "Support Form" ? (
                <>
                  ...
                  >
                    <li>
                      <span className="text-[#C3C3C3] text-2xl">●</span> You
                      have product or app-related questions
                    </li>
                    <li>
                      <span className="text-[#C3C3C3] text-2xl">●</span> You
                      need technical help or installation support
                    </li>
                    <li>
                      <span className="text-[#C3C3C3] text-2xl">●</span> You
                      want to report an issue or bug
                    </li>
                    <li>
                      <span className="text-[#C3C3C3] text-2xl">●</span> You
                      need help with FanDom or FanPedia features
                    </li>
                  </motion.ul>
              ...
              ) : activeTab === "Brand Partnership Form" ? (
                <>
                  <motion.h2
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <li>
                      <span className="text--[#C3C3C3] text-2xl">●</span>{" "}
                      Sponsorship opportunities
                    </li>
                    <li>
                      <span className="text--[#C3C3C3] text-2xl">●</span> Game
                      Room branding
                    </li>
                    <li>
                      <span className="text--[#C3C3C3] text-2xl">●</span>{" "}
                ...
                <>
                  ...
                  <motion.p
                    ...
                    className="text-[#C3C3C3]/70 font-semibold leading-8"
                  >
                    Thank you for your interest in partnering with Fanith for
                    the cricket season! Please fill out this quick form so we
                    can set up your creator profile and tracking details.
                  </motion.p>
                </>
              )}
    ...
    try {
      ...
      if (response && response.id && response.createdAt) {
        showPopup({
          title: "Success!",
          message:
            "Your partnership inquiry has been submitted successfully! We'll review it and get back to you soon.",
          type: "success",
        });
    ...
    try {
      ...
      if (response && response.id && response.createdAt) {
        showPopup({
          title: "Success!",
          message:
            "Your creator form has been submitted successfully!",
          type: "success",
        });
    ...
    >
        ...
        render={({ field }) => (
          <div className="space-y-2">
            <FormInput
              label="Fanith ID"
              {...field}
              error={errors.fanithId?.message}
            />

            <p className="text-xs text-[#C3C3C3]/70 mt-1">
              (If you haven't created your account yet, please download the app
              and set up your handle first)
            </p>
          </div>
        )}
      />
      ...
      <div className="space-y-4">
        ...
        {fields.map((item, index) => (
          ...
          >
            <Controller
              name={`socialMedia.${index}.socialLink`}
              control={control}
              render={({ field }) => (
                <Input
                  {...field}
                  placeholder="Instagram, YouTube, X, or other main platform where you post sports content"
                  className="h-8 md:h-10 lg:h-12 border-2 rounded-full text-white px-5 py-3 border-[#FFFFFF]/50"
                />
              )}
            />

            <Controller
              name={`socialMedia.${index}.followerCount`}
              control={control}
              render={({ field }) => (
                <Input
                  {...field}
                  placeholder="e.g., 50k on Instagram, 100k on YouTube"
                  className="h-8 md:h-10 lg:h-12 border-2 rounded-full text-white px-5 py-3 border-[#FFFFFF]/50"
                />
              )}
            />
        ...
        <button
          ...
          className="border-2 border-[#960018] px-6 py-2 rounded-lg font-medium text-[#960018] transition-all duration-300 ease-in-out hover:bg-transparent hover:text-white cursor-pointer"
        >
          Add More
        </button>
      </div>
  ...
  return (
    <div className="space-y-2">
      ...
      <Input
        ...
        type={type}
        className={`h-8 md:h-10 lg:h-12 border-2 rounded-full text-white px-5 py-3 ${
          error
  ...
  return (
    <div className="space-y-2">
      ...
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger
          className={`w-full h-8 md:h-10 lg:h-12 border-2 rounded-full text-white px-5 py-3 ${
            error
              ? "border-red-500 focus:border-red-500"
              : "border-[#FFFFFF]/50"
          }`}
...
function FormTextarea({
  ...
  return (
    <div className="space-y-2">
      ...
      <Textarea
        id={label.toLowerCase().replace(/\s+/g, "-")}
        className={`min-h-32 border-2 rounded-xl text-white px-5 py-3 ${
          error
            ? "border-red-500 focus:border-red-500"
            : "border-[#FFFFFF]/50"
        }`}
  ...
  return (
    <div className="pt-4 text-center md:text-right">
      <motion.button
        ...
        className={`px-10 py-2 rounded-lg font-semibold transform transition-all duration-300 ease-in-out ${
          isSubmitting
