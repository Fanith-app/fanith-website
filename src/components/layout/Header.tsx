  ...
  return (
    <>
      ...
      {/* <header className="fixed top-5 left-1/2 -translate-x-1/2 w-full max-w-6xl lg:w-[95%] bg-[#141414]/80 rounded-xl border border-[#eeeeee]/18 z-100 mx-auto"> */}
          ...
          <div className="flex h-16 items-center justify-between px-2 ">
            ...
            <nav className="hidden lg:flex items-center gap-10">
              {navLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-md font-normal transition ${isActiveLink(link.href)
                    ? "text-[#960018]" // active
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="lg:hidden p-2 rounded-md border bg-white transform transition-all duration-200 ease-in-out hover:scale-110 active:scale-95"
              >
        ...
        {/* Sidebar */}
        <div className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Mobile Menu */}
            {/* Header */}
